...
public class AuthControllerTests
{
    ...
    [Fact]
    public async Task Logout_WithInstallationId_RemovesOwnedDevice()
    {
        await using var factory = new CustomWebApplicationFactory();
        await factory.SeedMemberAsync();
        ...
        var verifyDb =
            verifyScope.ServiceProvider.GetRequiredService<AKContext>();

        Assert.Empty(await verifyDb.MobileDevices.ToListAsync());

        var session = await verifyDb.MobileSessions.SingleAsync();
        Assert.NotNull(session.RevokedAt);
    }

    [Fact]
    public async Task Logout_WithoutInstallationId_PreservesDevice()
    {
        await using var factory = new CustomWebApplicationFactory();
        await factory.SeedMemberAsync();
        ...
        Assert.Single(await verifyDb.MobileDevices.ToListAsync());
    }

    [Fact]
    public async Task Logout_UnknownRefreshToken_DoesNotRemoveDevice()
    {
        await using var factory = new CustomWebApplicationFactory();
        await factory.SeedMemberAsync();
