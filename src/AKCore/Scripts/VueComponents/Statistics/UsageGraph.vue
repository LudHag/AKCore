...
import { UsageResponse } from "./models";
import {
  Chart,
  CategoryScale,
  LinearScale,
...
import { getFeatureUsageLabel, getRandomColor } from "./utils";
import { computed } from "vue";
import Spinner from "../Spinner.vue";

const { dataPoints } = defineProps<{
  dataPoints: UsageResponse;
  loading: boolean;
}>();
  ...
  dataPoints
    ? {
        ...
        datasets: dataPoints.items.map((item) => {
          return {
            label: getFeatureUsageLabel(item.type),
            backgroundColor: getRandomColor(item.type),
            borderColor: getRandomColor(item.type),
