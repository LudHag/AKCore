...
function RecruitList(data, dom) {
  ...
  this.dom.append(
    $(
      '<div class="row recruit-head"><div class="col-sm-2">Skapad</div><div class="col-sm-3">Namn</div><div class="col-sm-2">Instrument</div><div class="col-sm-4 contact">Kontaktinformation</div><div class="col-sm-1 actions"></div>'
    )
  );
  this.render();
}
  ...
  this.dom.append(
    $(
      '<div class="col-sm-2">' +
        ...
        "-" +
        ("0" + (data.created.getMonth() + 1)).slice(-2) +
        "-" +
        ("0" + data.created.getDate()).slice(-2) +
        "</div>"
    )
  );
  this.dom.append(
    $('<div class="col-sm-3">' + data.fname + " " + data.lname + "</div>")
  );
...
Recruit.prototype.export = function() {
  ...
  return `${data.fname}\t${data.lname}\t${data.instrument}\t${data.email}\t${
    data.phone
  }`;
};
...
Recruit.prototype.filter = function() {
  ...
  const search = $("#interest-search")
    .val()
    .toLowerCase();
  if (search.length > 0) {
    if (
      (
        this.data.fname.toLowerCase() +
        " " +
        this.data.lname.toLowerCase()
      ).indexOf(search) < 0
    ) {
      this.hide();
      return;
